...
void setup() {
  ...
  if (!sdStatus || !sdActive) while(!updateButtonStatus()) wait(20);

  wait(DELAY_SETUP);

  while (!intTherm || !extTherm){                                                                   // Loop setup until it succeeds or user skips
    ...
    if (intTherm && extTherm) {                                                                     // Check if intTherm and extTherm were successfully initialized.
    ...
    if (intTherm && extTherm) break;
    while(!updateButtonStatus() && !updateSwitchStatus()) wait(20);                                 // wait for the user to make a choice
    if (updateButtonStatus()) break;                                                                // button skips 
    wait(100);
  }
  ...
  while (!msStatus) {                                                                               // Try to initialize the pressure sensor
    ...
    while(!updateButtonStatus() && !updateSwitchStatus()) wait(20);                                 // Wait for switch or button to be updated
  ...
  while(!bnoStatus){                                                                                // We are using the BNO055
    ...
    while(!updateButtonStatus() && !updateSwitchStatus()) wait(20);                                 // otherwise check for switch to be moved or button to be pressed
  ...
  while(!gpsStatus){                                                                                // We are using M9N GPS NOT in high-precision mode
    ...
    Screen.update(_print);
    if (gpsStatus) break;
    while(!updateButtonStatus() && !updateSwitchStatus()) wait(20);                                 // wait for button or switch to be updated
...
void loop() {
  ...
  if(millis() - xbeeTimerSeconds > DELAY_XBEE && isRelay) {                                         // appears to send data over xbee
...
bool xBeeSetup() {    // talk about a real mess
  ...
  while (updateButtonStatus() == 0){                
      ...
      while (switchPosition == updateSwitchStatus() && updateButtonStatus() == 0) wait(20); // wait for a change in switch status or a button press
  }
  counterID = 0;
  wait(300);
