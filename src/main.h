 ...
 * 
 * @note Unsure why specifically the GPS needs to be configured 50 times.
...
/**
 * Modifies the current period by input time in milliseconds, then returns the new frequency.
 * Stores the new period for next use.
 * @param _time Input in milliseconds
 * @returns Returns the modified frequency in Hz
 * @note Seems like a strange way to do this.
*/
